...
class AllTopUpHistory extends StatefulWidget {
  @override
  _AllTopUpHistoryState createState() => _AllTopUpHistoryState();
}

class _AllTopUpHistoryState extends State<AllTopUpHistory> implements MeterBillListContract{

  RefreshController _refreshController =RefreshController(initialRefresh: false);
  String nullResult="";
  String title="Account Balance Topup";
  List<AmountStatement> topupHistoryList=[];
  List<AmountStatement> sortByAmountList=[];
  List<String> sortType=["Amount","Date"];
  ...
  getCacheData()async{
    ...
    String res=await db.getData(DataKeyValue.topupList);
    if(res==null||res=="")
    {
      topupHistoryList=[];
    }else{
  ...
  getSortTypeName()async{
    String sortKey=await db.getData(DataKeyValue.sortTypeName);
    sortName=sortKey==null?sortName:sortKey;
    setState(() {
                    ...
                    InkWell(
                      ...
                      child: RadioListTile(
                        ...
                        onChanged:(newName){
                          ...
                          db.setData(sortName, DataKeyValue.sortTypeName);
      ...
      home: SafeArea(
        child: Scaffold(
          appBar: AppBar(
            ...
            title: Text(AppLocalizations.of(context).translate("topuphist"),style: TextStyle(fontSize: 17),),
            actions: <Widget>[
            ...
            Container(child: Column(children: <Widget>[
              Row(children: <Widget>[
                Container(
                  ...
                  child: Text(AppLocalizations.of(context).translate("topuphist1"),style: TextStyle(fontWeight: FontWeight.w500,color: Colors.grey)),
                ),
  ...
  void getAmountStatementSuccess(result) {
    if(result!=null){
      ...
      topupHistoryList=[];
      nullResult="There is no topup record to your account";
    }
    setState(() {
      onlineLoading=false;
      topupListLoading=false;
